...
namespace ApiScheduler.Controllers
{
    public class ShiftController : ApiController
    {
        ...
        public ShiftController()
        {
            ...
            _historyShiftService = new HistoryShiftService();
        ...
        [HttpPost]
        [Route("shifts")]
        public IHttpActionResult SetShift(ShiftPost shift)
        {
            ...
            if (!_user.check) return Ok(new { data = _user.user, message = _user.message, check = _user.check });
            if (!_ward.check) return Ok(new { data = _ward.ward, message = _ward.message, check = _ward.check });
        ...
        [HttpGet]
        [Route("shifts/{id}")]
        public IHttpActionResult GetShift(string id)
        {
            var _shift = _shiftService.Get(id);

            return Ok(new { data = _shift.shift, message = _shift.message, check = _shift.check });
        }
        [HttpGet]
        [Route("shifts/{day}/users/{userid}")]
        public IHttpActionResult GetShiftByDayUserID(int day, string userid)
        {
        ...
        public IHttpActionResult GetShiftByWardId(string id)
        {
            var _shift = _shiftService.GetByWardId(id);

            return Ok(new { data = _shift.shifts.OrderBy(x=>x.CreatedOn).ToList(), message = _shift.message, check = _shift.check });
        }
        ...
        public IHttpActionResult GetShiftByUsId(string id)
        {
